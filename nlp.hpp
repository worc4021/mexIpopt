...
private:
    ...
    std::vector<matlab::data::Array> feval(const matlab::data::Array &handle, const size_t numReturned, const std::vector<matlab::data::Array> &arguments) {
    ...
    bool isfield(matlab::data::StructArray& s, std::string fieldname) {
        bool retVal = true;
        try {
...
public:
    ...
    void setup(std::shared_ptr<matlab::engine::MATLABEngine>& mptr, matlab::data::TypedArray<double>& _x0, matlab::data::StructArray& _funcs, matlab::data::StructArray& _options) {
        ...
        if (isfield(funcs, "intermediate") && funcs[0]["intermediate"].getType() == ArrayType::HANDLE_OBJECT_REF){
  ...
  bool get_nlp_info(Index& n, Index& m, Index& nnz_jac_g,
            Index& nnz_h_lag, IndexStyleEnum& index_style) {
                ...
                assert (matlab::data::ArrayType::SPARSE_DOUBLE ==  jacStrOut[0].getType());
                ...
                if (returnHessian){
                    ...
                    assert( matlab::data::ArrayType::SPARSE_DOUBLE == hesStrOut[0].getType());
  ...
  bool get_starting_point(Index n, bool init_x, Number* x,
            ...
            Number* lambda) {
                ...
                if (init_z){
                    stream << "We are asked to initialise z" << std::endl;
                    for (i=0;i<n;i++){
                        z_L[i] = 0.;
                        z_U[i] = 0.;
                    }
                if (init_lambda) {
                    stream << "We are asked to initialise lambda" << std::endl;
                    for (i=0;i<m;i++)
                        lambda[i] = 0.;
                }
                      ...
                      Index* jCol, Number* values) {
                          ...
                          if (nullptr == values){
                            ...
                            std::vector<matlab::data::Array> hessianArgs = {
                                args[0], factory.createScalar(obj_factor), factory.createArray<double>({static_cast<size_t>(m),1}, lambda, lambda + m)
                            };
      ...
      if (intermediateCallback){
            ...
            if ( nullptr != ip_cq )
            {
                ...
                if ( nullptr != tnlp_adapter ){
                    passVal = factory.createStructArray({1, 1},{"primals", "duals", "duallbs", "dualubs","iter","obj_value","inf_pr","inf_du","mu","d_norm","regularization_size","alpha_du","alpha_pr"});
